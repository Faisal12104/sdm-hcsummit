...
const Berkas = () => {
  ...
  const handleAction = (id, action) => {
    ...
    if ((action === 'approve' || action === 'reject') && !window.confirm(`Yakin ingin ${action} berkas ini?`)) return;
    ...
    if (action === 'download') {
      ...
      setBerkasData(prev => ({
        ...
        [selectedSektor]: prev[selectedSektor].map(item =>
          item.id === id
            ? { ...item, status: action === 'approve' ? 'Approved' : 'Rejected', tanggalApprove: action === 'approve' ? new Date() : null }
            : item
        ),
      }));
    }
    ...
    <div className="dashboard-berkasadmin">
      ...
      <aside className={`sidebar ${isCollapsed ? 'collapsed' : ''} ${isSidebarOpen ? 'open' : ''}`}>
        ...
        <nav className="nav-links">
          ...
          <button onClick={() => navigate('/berkas')} className="active-link"><FaFileAlt /><span>Manajemen Berkas</span></button>
